...
## Definition
Aggregated realized profit for spends executed on the metric date.

## Formula
```
realized_profit_usd = Σ max(realized_value_usd − cost_basis_usd, 0) over daily spends.
```
...
## Reproduction
```sql
SELECT date, realized_profit_usd
FROM read_parquet('data/metrics/daily/metrics.parquet')
WHERE date BETWEEN '2024-01-01' AND '2024-01-10';
```

## Golden Day Snapshot
Capture the golden day chart or table and store it at `images/realized_profit_usd_golden.png` to replace this note.
