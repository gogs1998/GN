# Supply Cost Basis Usd

## Definition
Aggregate USD cost basis for the current UTXO set.

## Formula
```
Supply Cost Basis = Σ cost_basis_usd across current UTXO set.
```
...
## Reproduction
```sql
SELECT date, supply_cost_basis_usd
FROM read_parquet('data/metrics/daily/metrics.parquet')
WHERE date BETWEEN '2024-01-01' AND '2024-01-10';
```

## Golden Day Snapshot
Capture the golden day chart or table and store it at `images/supply_cost_basis_usd_golden.png` to replace this note.
