...
## Definition
Outstanding UTXO supply expressed in BTC units.

## Formula
```
Supply BTC = Σ balance_btc across current UTXO set.
```
...
## Reproduction
```sql
SELECT date, supply_btc
FROM read_parquet('data/metrics/daily/metrics.parquet')
WHERE date BETWEEN '2024-01-01' AND '2024-01-10';
```

## Golden Day Snapshot
Capture the golden day chart or table and store it at `images/supply_btc_golden.png` to replace this note.
