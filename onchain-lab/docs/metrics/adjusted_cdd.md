...
## Definition
Coin Days Destroyed normalized by spent volume for the metric date.

## Formula
```
Adjusted CDD = CDD / spent_value_btc.
```
...
## Reproduction
```sql
SELECT date, adjusted_cdd
FROM read_parquet('data/metrics/daily/metrics.parquet')
WHERE date BETWEEN '2024-01-01' AND '2024-01-10';
```

## Golden Day Snapshot
Capture the golden day chart or table and store it at `images/adjusted_cdd_golden.png` to replace this note.
