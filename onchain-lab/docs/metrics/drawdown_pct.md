...
## Definition
Percentage drawdown from trailing price maximum.

## Formula
```
Drawdown % = (price_close − rolling_max(price_close)) / rolling_max(price_close).
```
...
## Golden Day Snapshot
Capture the golden day chart or table and store it at `images/drawdown_pct_golden.png` to replace this note.
